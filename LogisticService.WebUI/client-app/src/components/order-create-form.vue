...
export default {
  ...
  methods: {
    ...
    generateAdminBlocks(contractAdminsBlocks) {
      if (contractAdminsBlocks.length < 1) {
        this.blocks.forEach(block => {
          contractAdminsBlocks.push({blockId: block.id,})
        })
      } else {
        this.blocks.forEach(block => {
          if (!contractAdminsBlocks.some(contractAdminBlock => contractAdminBlock.blockId === block.id)) {
            contractAdminsBlocks.push({blockId: block.id,})
          }
        })
      }
      return contractAdminsBlocks
    },
  },
  components: {
    DropDownServiceSelect,
